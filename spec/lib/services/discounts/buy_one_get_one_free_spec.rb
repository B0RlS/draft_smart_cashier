RSpec.describe Services::Discounts::BuyOneGetOneFree do
  ...
  describe '.calculate_discount_price' do
    ...
    context 'when quantity is less than 2' do
      let(:quantity) { 1 }
    ...
    context 'when quantity is greater than or equal to 2' do
      let(:quantity) { 3 }

      it 'returns discounted price' do
        expect(subject).to eq(Money.new(622, 'GBP'))
      end
    end
  end
